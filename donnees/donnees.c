...
int donneesControleur(controleurT * control)
	...
	projectionInitialisePointDeVue(&(*control).projection, 3*LARGEUR, 0.1 + PI/2, 0.1 + PI/2);//r, psi, phi
...
int donneesOptions(optionsT * options)
	...
	(*options).nombre=233;		// nombre de pendule
	(*options).equation=1;		// 1 : pendule, 2 : linéarisation,
							//	 3 : corde, 4 : dioptre

	(*options).dt=0.0001;		// discrétisation du temps
...
int donneesSysteme(systemeT * systeme, optionsT * options)
	...
	//return donneesTest(systeme) + erreur;
	return 0;
	}
...
int donneesTest(systemeT * systeme)
	{
	return 0;
	}
*/
